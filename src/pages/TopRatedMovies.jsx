...
export default function TopRatedMovies() {
  ...
  const {getTopRatedMoviesWithPaginateAndSort} = useMovies()
  const {mode} = useAuth()
  ...
  const fetchMovies = async (sortBy , currentPage) => {
    ...
    setError(null)
    try {
      const data = await getTopRatedMoviesWithPaginateAndSort(sortBy, currentPage)
  ...
  return (
    <div className={`bg-background text-foreground mx-auto px min-h-screen p-4 py-8 ${mode}`}>
      <h1 className="text-4xl font-bold mb-8 text-center">Top rated movies</h1>
